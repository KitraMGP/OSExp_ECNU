...
在`timer_interrupt_handler()`函数开头添加：

```c
printf("CPU%d: di da\n", mycpuid());
```

运行输出如下。两个 CPU 的输出顺序可能因中断到达时间不同而变化：
...
两个 CPU 均能周期性进入`timer_interrupt_handler()`，证明机器时钟中断、S-mode 软件中断转发和内核 trap 处理链路有效。

### 2. 时钟快慢测试

在`timer_update()`函数最后、释放计时器锁之后添加：
...
`ticks`持续递增，说明系统时钟能够在时钟中断中正常更新。修改`INTERVAL`宏定义可以改变时钟 tick 的更新频率：默认值`1000000`约为每 0.1 秒触发一次，修改为`5000000`后约为每 0.5 秒触发一次。

### 3. UART 输入测试

启动内核后向串口输入字符，测试普通字符、Backspace 和回车键。测试结果如下：

- 输入普通字符时，字符能够正常回显。
- 按下 Backspace 时，光标向左移动，并删除光标前的字符。
- 按下回车键时，光标能够移动到下一行行首。

当前 UART 输入功能仍较为原始，只实现了字符回显和简单的退格、换行处理，尚未维护完整的行输入缓冲区。光标可以通过终端控制序列任意移动，不会像常规行编辑器一样在行尾阻止继续右移；将光标移动到已有字符之间后，新输入的字符会覆盖后面的字符，而不会插入并将原有内容后移。
